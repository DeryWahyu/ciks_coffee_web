...
<div class="space-y-3 sm:space-y-0 sm:flex sm:flex-wrap sm:items-center sm:justify-between sm:gap-3 mb-6">
    <div class="flex flex-wrap items-center gap-2">
        <a href="{{ route('karyawan.customers.index', ['period' => 'all']) }}" class="px-3 sm:px-4 py-2 text-xs font-semibold rounded-xl border transition-all {{ $period === 'all' ? 'bg-espresso text-cream border-espresso' : 'bg-white text-caramel border-latte/50 hover:border-caramel' }}">Semua Waktu</a>
        <a href="{{ route('karyawan.customers.index', ['period' => 'month']) }}" class="px-3 sm:px-4 py-2 text-xs font-semibold rounded-xl border transition-all {{ $period === 'month' ? 'bg-espresso text-cream border-espresso' : 'bg-white text-caramel border-latte/50 hover:border-caramel' }}">30 Hari</a>
        <a href="{{ route('karyawan.customers.index', ['period' => 'week']) }}" class="px-3 sm:px-4 py-2 text-xs font-semibold rounded-xl border transition-all {{ $period === 'week' ? 'bg-espresso text-cream border-espresso' : 'bg-white text-caramel border-latte/50 hover:border-caramel' }}">7 Hari</a>
    </div>
    <form method="GET" action="{{ route('karyawan.customers.index') }}" class="flex items-center gap-2">
