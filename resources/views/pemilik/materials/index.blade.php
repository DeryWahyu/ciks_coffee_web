...
@section('content')
    <div class="w-full space-y-4 sm:space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-latte/50 overflow-hidden">
            <div class="hidden overflow-x-auto overscroll-x-contain lg:block">
                <table class="w-full min-w-[640px]" id="materials-table">
                    <thead>
                            ...
                            <tr class="hover:bg-cream/30 transition-colors" id="ingredient-{{ $item->id }}">
                                ...
                                <td class="px-4 py-4 text-center sm:px-6">
                                    <div class="flex items-center justify-center gap-1">
                                        ...
                                        <form method="POST" action="{{ route('pemilik.materials.destroy', $item) }}"
                                              data-ciks-confirm
                                              data-ciks-confirm-title="Hapus bahan baku?"
                                              data-ciks-confirm-message="Bahan baku ini akan dihapus permanen dan tidak dapat dipulihkan."
                                              data-ciks-confirm-action="Ya, Hapus Bahan"
            ...
            <div class="divide-y divide-latte/30 lg:hidden">
                @forelse ($ingredients as $item)
                    <article class="space-y-4 p-4 sm:p-5" id="ingredient-mobile-{{ $item->id }}">
                        <div class="flex items-start justify-between gap-3">
                            <div class="flex min-w-0 items-center gap-3">
                                <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-espresso/10">
                                    <svg class="h-5 w-5 text-espresso" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M20.25 6.375c0 2.278-3.694 4.125-8.25 4.125S3.75 8.653 3.75 6.375m16.5 0c0-2.278-3.694-4.125-8.25-4.125S3.75 4.097 3.75 6.375m16.5 0v11.25c0 2.278-3.694 4.125-8.25 4.125s-8.25-1.847-8.25-4.125V6.375"/></svg>
                                </div>
                                <p class="min-w-0 truncate text-sm font-semibold text-espresso">{{ $item->nama_bahan }}</p>
                            </div>
                            @if ($item->stok <= 10)
                                <span class="shrink-0 rounded-full bg-red-50 px-2 py-1 text-[0.65rem] font-bold uppercase tracking-wide text-red-600">Stok rendah</span>
                            @endif
                        </div>

                        <dl class="grid grid-cols-2 gap-3 rounded-xl bg-cream/40 p-3">
                            <div>
                                <dt class="text-[0.65rem] font-semibold uppercase tracking-wider text-caramel">Satuan</dt>
                                <dd class="mt-1 text-sm font-medium text-espresso">{{ $item->satuan }}</dd>
                            </div>
                            <div class="border-l border-latte/40 pl-3">
                                <dt class="text-[0.65rem] font-semibold uppercase tracking-wider text-caramel">Stok tersedia</dt>
                                <dd class="mt-1 text-sm font-bold {{ $item->stok <= 10 ? 'text-red-600' : 'text-espresso' }}">
                                    {{ number_format($item->stok, $item->stok == intval($item->stok) ? 0 : 2, ',', '.') }}
                                </dd>
                            </div>
                        </dl>
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <button type="button" onclick="editIngredient({{ $item->id }}, '{{ addslashes($item->nama_bahan) }}', '{{ $item->satuan }}', {{ $item->stok }})" class="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl bg-espresso/10 px-3 py-2 text-sm font-semibold text-espresso transition hover:bg-espresso/15">
                            ...
                            <form method="POST" action="{{ route('pemilik.materials.destroy', $item) }}"
                                              ...
                                              data-ciks-confirm-variant="danger">
                                @csrf @method('DELETE')
                                <button type="submit" class="inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-xl bg-red-50 px-3 py-2 text-sm font-semibold text-red-600 transition hover:bg-red-100">
                                    <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0"/></svg>
                                    Hapus
                                </button>
                            </form>
                ...
                @empty
                    <div class="px-6 py-12 text-center">
                        <div class="mx-auto mb-3 flex h-14 w-14 items-center justify-center rounded-2xl bg-latte/30">
                            <svg class="h-7 w-7 text-caramel" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M20.25 6.375c0 2.278-3.694 4.125-8.25 4.125S3.75 8.653 3.75 6.375m16.5 0c0-2.278-3.694-4.125-8.25-4.125S3.75 4.097 3.75 6.375m16.5 0v11.25c0 2.278-3.694 4.125-8.25 4.125s-8.25-1.847-8.25-4.125V6.375"/></svg>
                        </div>
                        <p class="text-sm font-medium text-caramel-dark">Belum ada bahan baku</p>
                        <p class="mt-1 text-xs text-caramel">Tambahkan bahan baku pertama menggunakan form di atas.</p>
                    </div>
                @endforelse
            </div>
