        ...
        <form method="POST" action="{{ route('pemilik.products.store') }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-latte/50 p-6 space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" required class="w-full px-4 py-2.5 text-sm bg-white border border-latte/60 rounded-xl focus:ring-2 focus:ring-caramel/30 focus:border-caramel outline-none transition" placeholder="Contoh: Cappuccino">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3" class="w-full px-4 py-2.5 text-sm bg-white border border-latte/60 rounded-xl focus:ring-2 focus:ring-caramel/30 focus:border-caramel outline-none transition resize-none" placeholder="Deskripsi singkat produk (opsional)">{{ old('description') }}</textarea>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <div id="image-preview-area" class="w-full h-40 border-2 border-dashed border-latte/60 rounded-xl flex flex-col items-center justify-center cursor-pointer hover:border-caramel/60 transition" onclick="document.getElementById('image').click()">
                            <div id="image-placeholder" class="flex flex-col items-center">
                                ...
                                <p class="text-xs text-caramel">Klik untuk upload gambar</p>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-latte/50 p-6 space-y-5">
                ...
                <div>
                    <label for="price" class="block text-sm font-semibold text-espresso mb-1.5">Harga <span id="price-label-type">(Base)</span> <span class="text-red-500">*</span></label>
                    <div class="relative">
                        <span class="absolute left-4 top-1/2 -translate-y-1/2 text-sm text-caramel font-medium">Rp</span>
                        <input type="number" name="price" id="price" value="{{ old('price') }}" required min="0" step="500" class="w-full pl-12 pr-4 py-2.5 text-sm bg-white border border-latte/60 rounded-xl focus:ring-2 focus:ring-caramel/30 focus:border-caramel outline-none transition" placeholder="0">
                    </div>
                    @error('price') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>

                <div id="lite-price-group" class="hidden">
                    <label for="price_lite" class="block text-sm font-semibold text-espresso mb-1.5">Harga Lite</label>
                    <div class="relative">
                        <span class="absolute left-4 top-1/2 -translate-y-1/2 text-sm text-caramel font-medium">Rp</span>
                        <input type="number" name="price_lite" id="price_lite" value="{{ old('price_lite') }}" min="0" step="500" class="w-full pl-12 pr-4 py-2.5 text-sm bg-white border border-latte/60 rounded-xl focus:ring-2 focus:ring-caramel/30 focus:border-caramel outline-none transition" placeholder="0">
                    </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-latte/50 p-6">
                <label class="flex items-center gap-3 cursor-pointer">
                    ...
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', '1') == '1' ? 'checked' : '' }} class="w-5 h-5 rounded-lg border-latte/60 text-espresso focus:ring-caramel/30">
                    <div>
