...
<style>
    ...
    @media (min-width:768px) { .owner-stat-grid { grid-template-columns:repeat(5,minmax(0,1fr)); } .owner-layout-grid { grid-template-columns:minmax(0,1fr) 17rem; } .owner-history { grid-column:span 2; } .owner-modal { align-items:center; } .owner-filter-grid { grid-template-columns:repeat(5,minmax(0,1fr)); } }
</style>
@endpush
...
<div class='owner-workspace'>
    ...
    <section id='owner-monitor' class='owner-monitor owner-layout-grid'>
        ...
        <aside class='owner-card'>
            <div class='owner-card-header'><div><h4 class='owner-card-title'>Detail meja</h4><p class='owner-card-subtitle'>Pilih satu meja pada denah.</p></div></div>
            <div id='owner-property' class='owner-property'><p class='owner-property-empty'>Pilih meja untuk melihat status, petugas terakhir, serta tindakan pengelolaan.</p></div>
        </aside>
    </section>
...
(() => {
    ...
    const state = { data: null, selectedId: null, editing: false, dirty: false, preview: 'desktop', loading: false, dragging: null, tableFormMode: 'create', statusTableId: null, historyPage: 1, historyMeta: null, toastTimer: null };
    const elements = {
    ...
    function renderMarker(marker) {
        const supported = ['counter', 'window', 'entrance'];
        const item = document.createElement('div');
        item.className = 'owner-marker ' + (supported.includes(marker.type) ? marker.type : 'marker');
    ...
    function renderTable(table) {
        ...
        button.setAttribute('aria-label', table.code + ', ' + table.capacity + ' kursi, ' + statusLabel(status));
    ...
    function renderProperty() {
        ...
        if (!table) {
            const empty = document.createElement('p'); empty.className = 'owner-property-empty'; empty.textContent = 'Pilih meja untuk melihat status, petugas terakhir, serta tindakan pengelolaan.'; elements.property.appendChild(empty); return;
        }
        ...
        actions.append(edit, status, toggle);
    ...
    function renderAll() {
        ...
        document.getElementById('owner-board-caption').textContent = state.editing ? 'Seret meja untuk mengubah posisi. Simpan setelah selesai.' : 'Klik meja untuk melihat detail dan tindakan.';
    }
    ...
    async function loadData({ quiet = false } = {}) {
        ...
        try {
            ...
            state.data = payload.data; state.selectedId = null; state.dirty = false; renderAll();
    ...
    function startDrag(event, table, button) {
        ...
        state.dragging = { id: table.id, button, startClientX: event.clientX, startClientY: event.clientY, startX: number(table.position_x), startY: number(table.position_y), rect };
    ...
    async function saveLayout() {
        ...
        try {
            const payload = { tables: state.data.tables.map((table) => ({ id: table.id, position_x: table.position_x, position_y: table.position_y, width: table.width, height: table.height, rotation: table.rotation, version: table.version })) };
    ...
    function toggleEditMode() {
        ...
        if (state.editing && state.dirty) { state.editing = false; loadData({ quiet: true }); return; }
